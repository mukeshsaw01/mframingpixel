...
async function getProjects() {
  try {
    const res = await fetch(`${API_URL}/api/architecture?depth=2&where[archive][not_equals]=true`, {
      cache: 'no-store',
    })
    ...
    return (
      <div className="min-h-[50vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Check back later for new architecture projects.</p>
        </div>
      </div>
    )
  }
      ...
      rows.push(
        <div key={`row-${i}`} className="flex gap-8 w-full">
          {[first, second].filter(Boolean).map((project) => {
            ...
            const alt = imageObj?.Alt || project?.title || 'Architecture Image'
            ...
            return (
              ...
              >
                <div
                  className="relative w-full h-[80vh] bg-gray-100"
                >
                  {imageUrl ? (
                    <Image
                      src={imageUrl}
