...
async function getProjects() {
  try {
    const res = await fetch(`${API_URL}/api/art?depth=2&where[archive][not_equals]=true`, {
      cache: 'no-store',
    })
    ...
    rows.push(
      <div key={`row-${i}`} className="flex gap-8 w-full">
        {[first, second].filter(Boolean).map((project) => {
          ...
          const alt = imageObj?.Alt || project?.title || 'Art Image'
    ...
    return (
      <div className="min-h-[50vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Check back later for new art projects.</p>
        </div>
      </div>
    )
  }
